...
select distinct q.id from tb_query_history q, tb_query_abstract a where q.id = a.id_query and ( a.id_abstract = 1 or a.id_abstract = 2 ) and q.availability <= 95.0 and q.response_time >= 2.0 and q.price_per_call >= 0.5 and (q.authentication = 'yes' or q.authentication = 'no') and (q.privacy = 'yes' or q.privacy = 'no') and (q.trust = 'low' or q.trust = 'medium') and q.degree_of_rawness = 'low' and (q.veracity = 'reliable' or q.veracity = 'unreliable') and q.production_time = 'working hours' and q.production_rate <= 2.0 and q.freshness = 'no' and q.provenance = 'not certified' and q.total_cost >= 10.0 and q.total_response_time >= 6.0 and ((select count(*) from tb_query_abstract a2 where a2.id_query = q.id) = 2);