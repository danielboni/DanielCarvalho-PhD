...
\textit{Concrete services} are defined in terms of \textit{abstract services} ($A_{1}, A_{2}, .., A_{n}$), and they include a set of service's quality aspects (quality features) ($M_{1},M_{2}, .., M_{g}$) that are extracted from the SLA exported by the service $S$.
%
%
...
Let us consider the following \textit{concrete services} specified using the \textit{abstract services} previously presented to be used as examples to the algorithm:%For example, consider the following \textit{concrete service} $S1$ which retrieves infected patients given a disease: 
...
%: $availability$ higher than 98 percent, $price \ per \ call$ less than 2 cents, and $total \ cost$ less than 5 dollars.
%
%\begin{figure}[h!]
%\center
...
%and (\textit{ii}) for all \textit{single preferences} $P_{i}$ in $Q$, there is one
...
%uilding combinations $I$ (Algorithm~\ref{rewriting}) deals with a NP hard complexity problem.
