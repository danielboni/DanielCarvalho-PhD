...
\textcolor{red}{CONTINUAR EXPLICANDO AS PREFERENCIAS}
...
\textcolor{red}{REVER DEFINIÇÃO QUE ESTÁ IGUAL A ANTERIOR}
...
\textcolor{red}{NÃO ESTÁ FALTANDO ALGO??? CONTINUAR EXPLICANDO}
...
(\textit{ii}) for all \textit{single measure} $P_{i}$ in $Q$, there is one
\textit{single measure} $Q_{i}$ in $S$ that satisfies the \textit{measures} matching problem (Definition 3).
\end{definition}
...
%\end{algorithmic}
%\end{algorithm}
\textcolor{red}{NÃO ESTÁ FALTANDO ALGO??? CONTINUAR EXPLICANDO O EXEMPLO}
