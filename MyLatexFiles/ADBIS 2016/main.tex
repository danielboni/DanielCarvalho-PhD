...
deal with to integrate data collections. Producing service compositions is
...
clouds. Our research focuses on how enhancing quality on data integration in
a cloud context.
This paper presents a data integration approach which takes into account user's
integration requirements while producinng and delivering her results. The
service selection and service composition are guided by the service level
agreement - SLA exported by different services (from one or more clouds).  In
addition, our matching algorithm (called \textit{Rhone}) that addresses query
rewriting for data integration is presented as proof of concept.
...
\input{related-works}
\section{SLA guided data integration: A running scenario}\label{sec:scenario}
%\input{scenario}
\input{section3}
