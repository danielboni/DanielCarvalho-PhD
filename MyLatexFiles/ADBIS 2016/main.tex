...
\begin{abstract}
Nowadays, data provision is mostly done by data services. Thus, data integration can be seen as composition of data services and data processing services that can lead to integrate data collections. Producing service compositions is computationally costly. Furthermore, executing them can require a considerable amount of memory, storage and computing resources that can be provided by clouds. Our research focus on how enhancing the quality on data integration in a cloud context.
This paper presents our data integration approach which takes into account user's integration requirements while producinng and delivering her results. The service selection and service composition are guided by the service level agreement exported by different services (from one or more clouds).  In addition, our matching algorithm (called \textit{Rhone}) that addresses
query rewriting for data integration is presented as proof of concept. 
